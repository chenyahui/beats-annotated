...
func (l *ackLoop) run() {
	...
	for {
		...
		case acks <- acked: // acks = l.broker.acks , 写入后，会被eventloop消费
			acks, acked = nil, 0

		case lst := <-l.broker.scheduledACKs: // eventloop会往这个channel里面灌数据，这样才能拿到最新的ack channel
...
func (l *ackLoop) handleBatchSig() int {
	...
	if count > 0 {
		// 这里的eventer实际上pipeline/pipeline.go里面的pipelineEventer
		// 最终会转发给filebeat的registrar
		if e := l.broker.eventer; e != nil {
			// 调用eventer的ack，count指的是buffer的length
			e.OnACK(count)
		}

		// report acks to waiting clients
		// 最终会调用eventloop中对应的processACK
