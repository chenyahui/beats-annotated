...
func newDirectEventLoop(b *Broker, size int) *directEventLoop {
	l := &directEventLoop{
		...
		acks:      b.acks, // acks = l.broker.acks , 写入后，会被eventloop消费
	}
...
func (l *directEventLoop) run() {
	...
	for {
		...
		case req := <-l.get: // consumer asking for next batch，
			// consumer会自己要求的batch大小，以及回包的channel，推送到broker.requests这个channel上
			l.handleConsumer(&req)

		case l.schedACKS <- l.pendingACKs: // 打通了ackloop和eventloop
...
func (l *directEventLoop) handleConsumer(req *getRequest) {
	...
	// 将取到的events回复给consumer
	req.resp <- getResponse{ackCH, buf}

	// pendingACKs会把消息发给schedACKS
	// 而schedACKS也是l.broker.scheduledACKs
	// 该channel也同时被ackloop监听着
	l.pendingACKs.append(ackCH)
	l.schedACKS = l.broker.scheduledACKs // 该channel被ackloop监听
}
