...
func newEventACK(
	...
	canDrop bool,
	sema *sema,
	fn func([]interface{}, int), // 对于filebeat来说，fn就是 pipelineEventCB.onEvents
) *eventDataACK {
...
// input把publish到pipeline之前，会先调用addEvent
// 具体调用处可以看client.go publish函数
...
func (a *eventDataACK) onACK(total, acked int) {
	...
	// 这算是个小优化，可以减少锁粒度，让锁的持有时间不依赖于fn的执行时长
	a.mutex.Lock()
	data := a.data[:n]
	a.data = a.data[n:]
	a.mutex.Unlock()
