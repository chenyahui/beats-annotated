...
func (p *Pipeline) makeACKer(
	canDrop bool, // canDrop = processors != nil, filebeat是true
	cfg *beat.ClientConfig,
	waitClose time.Duration,
	afterClose func(),
) acker {
	var (
		// 对于filebeat来说，ackBuilder是pipelineEventsACK
		bld   = p.ackBuilder
		acker acker
	)
	...
	default:
		if waitClose <= 0 {
			// waitClose = 0, switch case最终会走到这里
			// 所以，这里实际调用的是 pipelineEventsACK.createPipelineACKer
			// acker就是eventDataACK
