...
type pipelineEventsACK struct {
	pipeline *Pipeline
	cb       func([]interface{}, int) // 对于filebeat来说，cb就是pipelineEventCB.onEvents()
}
...
func newPipelineEventCB(handler beat.PipelineACKHandler) (*pipelineEventCB, error) {
	...
	cb := &pipelineEventCB{
		acks:          make(chan int),
		mode:          mode, // 对于filebeat来说，只有ACKEvents的callback, 所以mode是eventsACKMode
		handler:       handler,
		events:        make(chan eventsDataMsg),
		droppedEvents: make(chan eventsDataMsg),
	}

	// 开启一个worker，监听acks的事件
	go cb.worker()
...
func (p *pipelineEventCB) reportEventsData(data []interface{}, total int) {
	...
	case eventsACKMode: // filebeat是eventsACKMode
		// 这个handler就是filebeat通过b.Publisher.SetACKHandler
		// 最终会调用filebeat/beater/acker.go ackEvents()
