...
// client用以input放数据给pipeline的
// 在真正放数据到queue之前，会遍历processor
...
func (c *client) publish(e beat.Event) {
	...
	// 这里也很重要，publish之前，会像client_acker里面添加event
	...
	if c.canDrop {
		...
		// filebeat的canDrop是false
		published = c.producer.Publish(pubEvent)
	}

	// 调用一些callback，不重要，不用太关心
	if published {
		c.onPublished()
	} else {
		c.onDroppedOnPublish(e)
