...
// 类型与beat.Creator，相当于一个工厂函数, 最终返回一个beat.Beater
...
func (fb *Filebeat) Run(b *beat.Beat) error {
	...
	// 设置ACKHandler, filebeat只设置了ACKEvents的callback
	err = b.Publisher.SetACKHandler(beat.PipelineACKHandler{
		// 第一个参数是stateless, 第二个参数是stateful
	...
	crawler, err := crawler.New(
		channel.NewOutletFactory(outDone, wgEvents, b.Info).Create, // outlet的工厂函数
		config.Inputs,
		b.Info.Version,
		fb.done,
		*once)
