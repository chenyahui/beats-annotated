...
// 文档 https://www.elastic.co/guide/en/beats/filebeat/current/filebeat-input-log.html
type config struct {
	...
	IgnoreOlder    time.Duration   `config:"ignore_older"` // 忽略指定时间之前的文件
	Paths          []string        `config:"paths"`
	ScanFrequency  time.Duration   `config:"scan_frequency" validate:"min=0,nonzero"`
	CleanRemoved   bool            `config:"clean_removed"`
	HarvesterLimit uint32          `config:"harvester_limit" validate:"min=0"`
	Symlinks       bool            `config:"symlinks"`    // 默认false
	TailFiles      bool            `config:"tail_files"` // 默认false 表示，从最近修改的文件开始
	...
	ScanSort   string `config:"scan.sort"` // 取值：asc、desc，默认值asc
