...
func NewInput(
	cfg *common.Config,  // 该input对应的配置项
	outlet channel.Connector,  // pipelineConnector
	...
	// outlet的真实类型为pipelineConnector
	// 返回值是channel.outlet, 实际上是beat.Client的一个wrapper。
	// beat.Client用于和pipeline通信
	// 注意，这里的PublishMode没有设，就是默认的。
...
// 把 FileInfo[]，转换成 FileSortInfo[], 并不是排序的意思
...
// scan会遍历文件，并且会给每个文件都启动一个harvester
func (p *Input) scan() {
	...
	// 根据指定的ScanSort和ScanOrder, 对文件进行排序
...
// 检测某个文件的更改时间是否达到了clean_inactive
...
func (p *Input) createHarvester(state file.State, onTerminate func()) (*Harvester, error) {
	...
	h, err := NewHarvester(
		...
		subOutletWrap(p.outlet), // outletFactory
	)
	if err == nil {
